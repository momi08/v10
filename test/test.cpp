...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(test_04b)
		{
			...
			auto res = std::accumulate(std::begin(v), std::end(v), std::string("GO "));
		...
		TEST_METHOD(test_04c)
		{
			...
			auto total_age = std::accumulate(std::begin(v), std::end(v), 0, [](int age, person const& p) { return age + p.age; });
